...
public class MFMDrawController implements OperationChangeListener {
	...
	public synchronized static MFMDrawController getDrawController() {
		if (null == mfmDrawController) {
			...
			((OperationChangeController) OperationChangeController
					.getOperationChangeNotifier()).subscribe(mfmDrawController);
		}
	...
	private void clearNetwork() throws Exception {
		...
		NetworkBluePrintImpl.getNetWorkBluePrint().clearNetwork();
	}

	public void showHelp() {
		MFMSimulatorApplication myMFM = MFMSimulatorApplicationImpl
				.getMFMApplication();
		JOptionPane.showMessageDialog(
				((MFMSimulatorApplicationImpl) myMFM).getMFMFrame(),
				"Version 0.0.6"
						+ "\nAuthor(s) : Prem, Darshan, Pankaj & Christoph"
						+ "\nIs MFM helpfull: Let's see.  All is well!!!");
	}

	public void addSites(final float x1, final float y1) {
		...
		if (myCurrent_Operation == Operation_States.CELLS) {
			...
			for (Site singleSite : myNetworkBluePrint.getSitesWithCoOrdinates()) {
				final int totalCells = 1 + (new Random()
						.nextInt(NO_OF_SECTORS_PER_SITE));
				final List<Short> bearing = getBearings(totalCells);
				for (int cellNumber = 0; cellNumber < totalCells; cellNumber++) {
					int cellID = SequenceGenerator.getSideIDGenerator()
							.getNextCellID();
					myNetworkBluePrint.addCell(singleSite.getSiteId(), cellID,
							bearing.get(cellNumber).shortValue());
		...
		for (int cellNumber = 0; cellNumber < totalCells; cellNumber++) {
			if (NO_OF_SECTORS_PER_SITE < 5) {
				...
				case 0:
					...
					break;
				case 1:
					bearings.add(new Short(
							(short) (90 + (short) (Math.random() * (new Random()
									.nextInt(90))))));
					break;
				case 2:
					bearings.add(new Short((short) (180 + (short) (Math
							.random() * (new Random().nextInt(90))))));
					break;
				case 3:
					bearings.add(new Short((short) (270 + (short) (Math
							.random() * (new Random().nextInt(90))))));
					break;
				default:
					break;
